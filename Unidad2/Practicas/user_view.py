...
from user_controller import ver_usuarios,actualizar_usuario,agregar_usuarios, eliminar_usuario
...
class UserApp:
    ...
    def crear_elementos(self):
        tk.Label(self.root, text=f"Hola, {self.username}", font=("Arial",22, "bold")).pack(padx=10)

        tk.Button(self.root, text="Cerrar Sesion", command=self.cerrar_sesion).pack(pady=25)

        frame_botones = tk.Frame(self.root)
        frame_botones.pack(pady=10) 

        tk.Button(frame_botones, text="Agregar usuario", command=self.agregar_usuario).pack(pady=25)
    ...
    def eliminar_usuario(self):
        ...
        if confirmacion:
            if eliminar_usuario(user_id):
                ...
                messagebox.showerror("Error, no se pudo eliminar al usuario correctamente")
